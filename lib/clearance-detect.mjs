// Clearance + placement_type vision detection, shared by the CLI script
// (scripts/backfill-clearance-placement.mjs) and the server-side runner
// (api/backfill-clearance-placement.js). Requires ANTHROPIC_API_KEY.
...
// Detect clearance/placement for a set of observations and PATCH the results.
//   opts.shopOutId — restrict to one shop-out (re-detect all of its obs)
//   (else)         — observations with placement_type IS NULL
//   opts.limit     — cap rows processed this invocation
export async function runBackfill(opts = {}) {
  const log = opts.log || (() => {});
  if (!process.env.ANTHROPIC_API_KEY) throw new Error('ANTHROPIC_API_KEY not set');

  // Un-detected rows carry a NULL clearance_confidence (idempotent + paginatable).
  ...
  if (!Array.isArray(obs) || !obs.length) { log('Nothing to process.'); return { processed: 0, clearance: 0, placement: {}, errors: 0 }; }
