...
//
// Requires ANTHROPIC_API_KEY (so it runs server-side — see
// api/backfill-clearance-placement.js, which imports runBackfill). For each
// observation it signs the front photo, sends it to the AI, and PATCHes
// is_clearance / clearance_confidence / placement_type. Logs progress + a
// summary (total processed, clearance found, placement breakdown).
...
// Sign a Supabase-resized version of the photo (≤1568px) and return base64.
...
export async function runBackfill(opts = {}) {
  ...
  if (opts.shopOutId) q += '&shop_out_id=eq.' + opts.shopOutId;       // one shop-out (re-detect all)
  else q += '&placement_type=is.null';                                // literal: only un-set rows
  if (opts.limit) q += '&limit=' + opts.limit;
  const obs = await (await sb(q)).json();
  if (!obs.length) { log('Nothing to process.'); return { processed: 0, clearance: 0, placement: {}, errors: 0 }; }

  // Front-photo file paths.
...
// CLI entry (not triggered when imported by the API endpoint).
if (process.argv[1] && process.argv[1].includes('backfill-clearance-placement')) {
  const shopOutId = process.argv[2] || null;
  runBackfill({ shopOutId, log: console.log }).then(s => {
    console.log('\nClearance/placement backfill' + (shopOutId ? ' — shop_out ' + shopOutId : ''));
    console.log('  total processed: ' + s.processed);
    console.log('  clearance found: ' + s.clearance);
    console.log('  placement breakdown: ' + JSON.stringify(s.placement));
