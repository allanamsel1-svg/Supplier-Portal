// api/factory-extract.js
// Extract structured factory contact fields from pasted free-text via Claude.
//   POST { text: string }   Auth: Bearer <tenant session token | admin session token>
//   → { factory_name_english, factory_name_chinese, contact_name, email, phone, wechat_id,
//       address, city, country, website, product_categories: [], tenant_id } | { error, message }
...
const SYSTEM = "You are a factory data analyst. Extract factory contact information from the provided text. Return ONLY valid JSON with these fields: factory_name_english (string), factory_name_chinese (string or null), contact_name (string or null), email (string or null), phone (string or null), wechat_id (string or null), address (string or null), city (string or null), country (string or null), website (string or null), product_categories (array of strings). If a field is not found, use null. No markdown, no backticks.";
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!text) return res.status(400).json({ error: true, message: 'No text provided' });
    ...
    try {
      r = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({ model: MODEL, max_tokens: 600, system: SYSTEM, messages: [{ role: 'user', content: text }] }),
      });
    ...
    logCost(tid, 'factory_extract', MODEL, d.usage);
    ...
    parsed.tenant_id = tid;   // so the client can insert the factory under the right tenant
