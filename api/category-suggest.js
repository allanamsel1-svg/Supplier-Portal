// api/category-suggest.js
// Recommend where a suggested category name belongs in the taxonomy.
//   POST { suggested_name, existing_categories:[names] }   Auth: Bearer <tenant session | admin session>
//   → { recommendation, suggested_parent, suggested_sub, reasoning, confidence } | { error, message }
// Cost is logged to api_cost_log (1.5x markup) the same way api/hts-suggest.js does.
...
const SYSTEM = "You are a product category taxonomy expert. Given a suggested category name and a list of existing categories, recommend where it should be placed in the hierarchy. Return ONLY valid JSON: { recommendation: 'top_level|sub_category|sub_sub_category', suggested_parent: 'existing parent name or null', suggested_sub: 'existing sub name or null', reasoning: 'one sentence', confidence: 'high|medium|low' }";
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const name = (body.suggested_name || '').toString().trim();
    let existing = Array.isArray(body.existing_categories) ? body.existing_categories : [];
    existing = existing.filter(Boolean).map(x => x.toString().trim()).filter(Boolean);
    if (!name) return res.status(400).json({ error: true, message: 'suggested_name required' });
    ...
    try {
      r = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({ model: MODEL, max_tokens: 200, system: SYSTEM,
          messages: [{ role: 'user', content: 'Suggested: ' + name + '. Existing categories: ' + (existing.join(', ') || '(none)') }] }),
      });
    ...
    logCost(tid, 'category_suggest', MODEL, d.usage);

    const parsed = parseJson((d.content && d.content[0] && d.content[0].text) || '');
    if (!parsed || !parsed.recommendation) return res.status(200).json({ error: true, message: 'Could not parse AI response' });
